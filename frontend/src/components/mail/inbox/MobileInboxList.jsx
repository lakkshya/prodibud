...
const MobileInboxList = ({ data }) => {
  ...
  const fetchResults = useCallback(
    async (value) => {
      ...
      try {
        const res = await axios.get("http://localhost:5000/api/search/inbox", {
          headers: {
    ...
    [data] // refetch logic depends on current inbox
  );

  // memoized debounced version of fetchResults
  const debouncedSearch = useMemo(
    () => debounce(fetchResults, 500),
    [fetchResults] // re-create if inbox data changes
  );
  ...
  useEffect(() => {
    // When `data` changes (fresh inbox load), reset results
    setResults(data);
  }, [data]);
