...
const MobileSentList = ({ data }) => {
  ...
  const fetchResults = useCallback(
    async (value) => {
      ...
      try {
        const res = await axios.get("http://localhost:5000/api/search/sent", {
          headers: {
    ...
    [data] // refetch logic depends on current sent
  );

  // memoized debounced version of fetchResults
  const debouncedSearch = useMemo(
    () => debounce(fetchResults, 500),
    [fetchResults] // re-create if sent data changes
  );
  ...
  useEffect(() => {
    // When `data` changes (fresh sent load), reset results
    setResults(data);
  }, [data]);
