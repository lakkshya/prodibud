...
const MobileDraftsList = ({ data }) => {
  ...
  const fetchResults = useCallback(
    async (value) => {
      ...
      try {
        const res = await axios.get("http://localhost:5000/api/search/drafts", {
          headers: {
    ...
    [data] // refetch logic depends on current drafts
  );

  // memoized debounced version of fetchResults
  const debouncedSearch = useMemo(
    () => debounce(fetchResults, 500),
    [fetchResults] // re-create if drafts data changes
  );
  ...
  useEffect(() => {
    // When `data` changes (fresh drafts load), reset results
    setResults(data);
  }, [data]);
  ...
  const renderContent = () => {
    if (!data || data.length === 0) {
      // draft is totally empty
      return (
        <div className="flex justify-center items-center p-6 text-gray-500">
          <p>Your drafts is empty</p>
        </div>
      );
    }
